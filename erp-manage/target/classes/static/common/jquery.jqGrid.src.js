...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		if(this.tagName.toUpperCase() !== 'TABLE' || this.id == null) {
			alertc("Element is not a table or has no id!");
			return;
		}
		if(document.documentMode !== undefined ) { // IE only
			if(document.documentMode <= 5) {
				alertc("Grid can not be used in this ('quirks') mode!");
				return;
			}
		}
...
var xmlJsonClass = {
	...
	toObj: function(xml) {
		...
		if (xml.nodeType === 1) {
			...
			alertc("unhandled node type: " + xml.nodeType);
		}
		return o;
	},
...
"use strict";	
	...
	$.unformat.select = function (cellval,options,pos,cnt) {
		...
		// cnt_manage is set to true only in sortDataArray
		var ret = [];
			...
			if ($.ui.multiselect ) {
				if(opts.msel === "multiselect") {
					if(!$.jgrid._multiselect) {
						...
						alertc("Multiselect plugin loaded after jqGrid. Please load the plugin before the jqGrid!");
						return;
					}
